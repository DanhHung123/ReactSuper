...
export default function Students() {
  ...
  const hanleDelete = (id: number) => {
    deleteStudentMutation.mutate(id, {
      onSuccess: (_, id) => {
        toast.success(`Delete success for id: ${id}`)
  ...
  return (
    <div>
      ...
      <div className='mt-6'>
        <Link to='/students/add' className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2">
          Add student
        </Link>
      </div>
      {isLoading &&
            ...
            <table className='w-full text-left text-sm text-gray-500 dark:text-gray-400'>
              ...
              <tbody>
                {data?.data.map((student: any) => (
                  <tr key={student.id} onMouseEnter={() => handlePrefetchStudent(student.id)} className='border-b bg-white hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-600'>
                    ...
                    <td className='py-4 px-6 text-right'>
                      ...
                      <button onClick={() => hanleDelete(student.id)} className='font-medium text-red-600 dark:text-red-500'>Delete</button>
                    </td>
                  </tr>
                ...
                </li>
                {
                  Array(totalPage).fill(0).map((_, index) => {
                    ...
                    return (
                      <li key={pageNumber}>
                        <Link
                          className={isActive ? 'border border-gray-300 bg-gray-200 py-2 px-3 leading-tight text-gray-700  hover:bg-gray-100 hover:text-gray-700'
